...
// Các chân nút nhấn
int button1 = 10;  // Nút dừng
int button2 = 2;   // Nút tiếp tục

// Biến trạng thái nút và logic điều khiển
...
void loop() {
  // Kiểm tra trạng thái nút nhấn
    if (digitalRead(button1) == LOW) {  // Nút dừng được nhấn
      isStopped = true;
      moveStop();
      Serial.println("Robot stopped");
    }
    if (digitalRead(button2) == LOW) {  // Nút tiếp tục được nhấn
      isStopped = false;
      Serial.println("Robot resumed");
    }

    // Nếu robot bị dừng, không thực hiện logic tiếp theo
    if (isStopped) return;
