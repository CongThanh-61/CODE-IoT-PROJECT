...
int leftSwitch = 2;   // Công tắc bên trái
int rightSwitch = 8; // Công tắc bên phải
...
const int LeftSpeed = 110;
const int RightSpeed = 110;
...
void loop() {
  ...
  bool leftSwitchState = digitalRead(leftSwitch);
  bool rightSwitchState = digitalRead(rightSwitch);

  // Nếu công tắc bên trái được kích hoạt
  ...
  // Nếu công tắc bên phải được kích hoạt
  ...
  if (leftDistance < DIS && leftFrontDistance < DIS && middleDistance < DIS && rightFrontDistance < DIS && rightDistance < DIS) {
    ...
    moveBackward();
    delay(600);
    if (leftFrontDistance > rightFrontDistance && leftDistance > rightDistance) {
      turnLeft();
      delay(500); // Quay trái
    } else {
      turnRight();
